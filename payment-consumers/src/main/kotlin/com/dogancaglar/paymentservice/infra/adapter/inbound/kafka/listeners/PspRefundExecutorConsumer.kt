    ...
    fun onPspRequested(
        ...
        EventLogContext.with(envelope) {
            ...
            val updatedOrder = when (pspStatus) {
                PaymentOrderStatus.REFUNDED -> current.markAsRefunded()
                PaymentOrderStatus.REFUND_FAILED -> current.markRefundDeclined()
                else -> current
            }
            paymentOrderModificationPort.updateReturningIdempotent(updatedOrder)

            val result = try {
                if (pspStatus == PaymentOrderStatus.REFUNDED) {
                    PaymentOrderRefunded.from(updatedOrder, Utc.nowInstant())
                } else {
                    PaymentOrderFinalized.from(updatedOrder, Utc.nowInstant(), pspStatus)
                }
            } catch (ex: Exception) {
                logger.error(
                    "🚨 Unexpected error building EVENT for $PAYMENT_ORDER_ID ${eventData.publicPaymentOrderId} " +
