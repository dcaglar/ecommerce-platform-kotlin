...
@Component
class PspRefundExecutorConsumer(
    ...
    private val pspLatency: Timer = Timer.builder("psp_refund_call_latency")
        .publishPercentileHistogram()
        .register(meterRegistry)

    @KafkaListener(
        topics = [Topics.REFUND_QUEUE],
        containerFactory = "${Topics.REFUND_QUEUE}-factory",
        groupId = CONSUMER_GROUPS.PSP_REFUND_EXECUTOR
    )
    fun onPspRequested(
        record: ConsumerRecord<String, EventEnvelope<PaymentOrderRefundCommand>>,
        ...
        EventLogContext.with(envelope) {
            ...
            if (current.status != PaymentOrderStatus.REFUND_REQUESTED && current.status != PaymentOrderStatus.PENDING_REFUND) {
                logger.warn(
                    "⚠️ Skip PSP: dbStatus=${current.status.name} expected=REFUND_REQUESTED or PENDING_REFUND for $PAYMENT_ORDER_ID ${eventData.publicPaymentOrderId} " +
                            "with $PAYMENT_ID ${eventData.publicPaymentId}")
                kafkaTx.run(offsets, groupMeta) {}
                return@with
            }
            ...
            val pspStatus = psp.refund(current)
            val tookMs = System.currentTimeMillis() - startMs
            pspLatency.record(tookMs, TimeUnit.MILLISECONDS)

            // 4. BUILD UPDATED EVENT (We can reuse PaymentOrderPspResultUpdated for now, or just send a dummy if it doesn't match perfectly, but PspResultUpdated works great)
            val result = try {
                // To reuse PaymentOrderPspResultUpdated we need to adapt the command
                // or just use a generic event. We'll use PaymentOrderPspResultUpdated for symmetry.
                ...
                logger.error(
                    "🚨 Unexpected error building PSP_RESULT_UPDATED for $PAYMENT_ORDER_ID ${eventData.publicPaymentOrderId} " +
