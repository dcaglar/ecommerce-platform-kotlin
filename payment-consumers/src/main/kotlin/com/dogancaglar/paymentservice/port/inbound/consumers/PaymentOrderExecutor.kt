...
class PaymentOrderExecutor(
    ...
    fun onMessage(record: ConsumerRecord<String, EventEnvelope<PaymentOrderCreated>>) {
        ...
        if (order.status != PaymentOrderStatus.INITIATED) {
            kafkaTx.run(offsets, CONSUMER_GROUPS.PAYMENT_ORDER_CREATED) { /* no-op */ }
        ...
        // Kafka TX: use case (DB state + publish) + offsets
        kafkaTx.run(offsets, CONSUMER_GROUPS.PAYMENT_ORDER_CREATED) {
