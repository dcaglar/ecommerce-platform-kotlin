...
class PspModificationGatewayAdapter(
    ...
    override fun refund(order: PaymentOrder): PaymentOrderStatus {
    ...
    private fun doRefund(): PaymentOrderStatus {
        try {
        ...
        val result = when {
            roll < active.response.successful -> PaymentOrderStatus.REFUNDED
            roll < active.response.successful + active.response.retryable -> PaymentOrderStatus.PENDING_REFUND
            roll < active.response.successful + active.response.retryable + active.response.nonRetryable -> PaymentOrderStatus.REFUND_DECLINED_FINAL
            else -> PaymentOrderStatus.PENDING_REFUND
        }
        return result
    }
