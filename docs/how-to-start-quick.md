...
Recommended order(clear exsiting leftover)
0) ```
   bash minikube-nuke-dev.sh
```
1) Bootstrap a local Kubernetes cluster
- What: Creates/uses a minikube profile sized from configured Docker resources and enables metrics-server
- Run:
```bash
