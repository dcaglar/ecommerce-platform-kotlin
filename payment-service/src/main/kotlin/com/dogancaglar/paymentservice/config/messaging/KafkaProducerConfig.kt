...
class KafkaProducerConfig(
    ...
    fun paymentOrderEventProducerFactory(meterRegistry: MeterRegistry): DefaultKafkaProducerFactory<String, EventEnvelope<*>> {
        // 1) Start with all spring.kafka.producer.* properties
        val props = bootKafkaProps.buildProducerProperties()
        // 2) Ensure we serialize keys as strings and values as JSON
        props[ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG] = StringSerializer::class.java
        props[ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG] = JsonSerializer::class.java
        // 3) Tell JsonSerializer to add type info (so it picks up your EventEnvelope<?>)
