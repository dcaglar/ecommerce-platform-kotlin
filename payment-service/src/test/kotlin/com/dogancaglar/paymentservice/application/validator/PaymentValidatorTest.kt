...
class PaymentValidatorTest {
    ...
    fun `validate should pass for valid payment request`() {
        ...
        val request = CreatePaymentRequestDTO(
            ...
            paymentOrders = listOf(
                PaymentLineDTO(sellerId = "seller-1", amount = AmountDto(quantity = 10000L, currency = CurrencyEnum.EUR)),
                PaymentLineDTO(sellerId = "seller-2", amount = AmountDto(quantity = 10000L, currency = CurrencyEnum.EUR))
            )
        )
    ...
    fun `validate should throw when payment order currency differs from total amount currency`() {
        ...
        val request = CreatePaymentRequestDTO(
            ...
            orderId = "ORDER-123",
            paymentOrders = listOf(
                PaymentLineDTO(sellerId = "seller-1", amount = AmountDto(quantity = 10000L, currency = CurrencyEnum.USD))
            )
        )
    ...
    fun `validate should validate all payment orders for multiple sellers`() {
        ...
        val request = CreatePaymentRequestDTO(
            ...
            paymentOrders = listOf(
                PaymentLineDTO(sellerId = "seller-1", amount = AmountDto(quantity = 10000L, currency = CurrencyEnum.EUR)),
                PaymentLineDTO(sellerId = "seller-2", amount = AmountDto(quantity = 10000L, currency = CurrencyEnum.EUR)),
                PaymentLineDTO(sellerId = "seller-3", amount = AmountDto(quantity = 10000L, currency = CurrencyEnum.EUR))
            )
        )
