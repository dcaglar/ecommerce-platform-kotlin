...
data class PaymentOrderRefundCommand private constructor(
    ...
    companion object {
        const val EVENT_TYPE = "payment_order_refund_requested"

        fun from(order: PaymentOrder, now: Instant,attempt: Int): PaymentOrderRefundCommand {
            require(order.status == PaymentOrderStatus.REFUND_REQUESTED || order.status== PaymentOrderStatus.PENDING_REFUND ){
                "Invalid PaymentOrderCommandGeneration creation, payment order status was ${order.status.name}"
            }
            return PaymentOrderRefundCommand(
                ...
                sellerId = order.sellerId.value.toString(),
