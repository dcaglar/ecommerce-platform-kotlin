...
class PaymentOrderDomainEventEntityMapperTest {
    ...
    fun `toPaymentOrderCaptureCommand maps retry count`() {
        ...
        val event = PaymentOrderDomainEventEntityMapper.toPaymentOrderCaptureCommand(order, attempt = 3)
    ...
    fun `toPaymentOrderSucceeded maps correctly`() {
        ...
        val event = PaymentOrderDomainEventEntityMapper.toPaymentOrderFinalized(order, now, PaymentOrderStatus.CAPTURED)
    ...
    fun `toPaymentOrderFinalized maps correctly for failed status`() {
        val order = sampleOrder()
        val now = Utc.nowLocalDateTime()
        val event = PaymentOrderDomainEventEntityMapper.toPaymentOrderFinalized(order, now, PaymentOrderStatus.CAPTURE_FAILED)
