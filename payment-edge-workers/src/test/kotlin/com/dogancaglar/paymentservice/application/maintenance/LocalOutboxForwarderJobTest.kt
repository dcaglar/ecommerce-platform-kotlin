...
class LocalOutboxForwarderJobTest {
    ...
    fun `forwardBatch should forward PaymentAuthorized events and update watermark`() {
        ...
        every { watermarkPort.updateWatermark(any(), any()) } just Runs
        ...
        verify(exactly = 1) {
            watermarkPort.updateWatermark("test-edge-node", Utc.toInstant(outboxEvent.createdAt))
        }
    }
    ...
    fun `forwardBatch should return false when insert fails`() {
        ...
        assertFalse(result)
        verify(exactly = 0) { watermarkPort.updateWatermark(any(), any()) }
    }
    ...
    fun `dispatchBatchWorker should claim, forward, and persist successfully`() {
        ...
        verify(exactly = 1) { watermarkPort.updateWatermark(any(), any()) }
